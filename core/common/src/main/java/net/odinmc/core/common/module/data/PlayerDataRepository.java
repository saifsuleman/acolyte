...
public class PlayerDataRepository {
    ...
    public Promise<PlayerData> create(UUID uuid, String name, InetAddress address, String serverAddress) {
        ...
        return database.executeGeneratedKeys("INSERT INTO players (uuid_upper_bits, uuid_lower_bits, register_name, register_time, register_address,"
                + " register_server_address, last_login_name, last_login_time, last_login_address, last_server_address)"
                + " VALUES (?, ?, ?, ?, INET_ATON(?), VALUES(register_name), VALUES(register_time), VALUES(register_address), VALUES(register_server_address))", statement -> {
            ...
            try {
                ...
                return new PlayerData(
                        result.getInt(1), uuid, name, time, address, serverAddress, name, time, address, null, serverAddress
                );
    ...
    public Promise<PlayerData> getById(int id) {
        return database.executeQuery("SELECT uuid_upper_bits, uuid_lower_bits, register_name, register_time, INET_NTOA(register_address), "
                + "register_server_address, last_login_name, last_login_time, INET_NTOA(last_login_address), last_logout_time "
                + "FROM players WHERE id = ?", statement -> {
    ...
    public Promise<PlayerData> getByUUID(UUID uuid) {
        return database.executeQuery("SELECT id, register_name, register_time, INET_NTOA(register_address), register_server_address, "
                + "last_login_name, last_login_time, INET_NTOA(last_login_address), last_login_server_address, last_logout_time "
                + "FROM players WHERE uuid_upper_bits = ? AND uuid_lower_bits = ?", statement -> {
            statement.setLong(1, uuid.getMostSignificantBits());
            statement.setLong(2, uuid.getLeastSignificantBits());
        }).thenApplyAsync(result -> {
            try {
    ...
    public Promise<PlayerData> getByLastLoginName(String name) {
        return database.executeQuery("SELECT id, uuid_upper_bits, uuid_lower_bits, register_name, register_time, INET_NTOA(register_address), "
                + "register_server_address, last_login_time, INET_NTOA(last_login_address), last_login_server_address, last_logout_time "
                + "FROM players WHERE last_login_name = ? ORDER BY last_login_time DESC LIMIT 1", statement -> {
    ...
    public Promise<Boolean> updateLogin(int id, String name, long lastLoginTime, InetAddress address, String serverAddress) {
        return database.executeUpdate("UPDATE players SET last_login_name = ?, last_login_time = ?, last_login_address = INET_ATON(?), "
                + "last_login_server_address = ? WHERE id = ?", statement -> {
            statement.setString(1, name);
    ...
    public Promise<Boolean> updateLogout(int id) {
        return database.executeUpdate("UPDATE players SET last_logout_time = NOW() WHERE id = ?", statement -> {
            statement.setInt(1, id);
        }).thenApplyAsync(result -> result > 0);
    }
}
